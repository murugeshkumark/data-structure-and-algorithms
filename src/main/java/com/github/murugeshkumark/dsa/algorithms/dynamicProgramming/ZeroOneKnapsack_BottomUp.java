...
public class ZeroOneKnapsack_BottomUp {
    ...
    public int solveKnapsack(int[] profits, int[] weights, int capacity) {
        ...
        int numberOfRows = profits.length + 1;// adding a dummy row to avoid com.github.murugeshkumark.dsa.array index issues
        int[][] dp = new int[numberOfRows][capacity + 1];// Create a 2D Array to store all the results

        for (int i = 0; i < numberOfRows; i++) { // Insert 0 in 1st column as it is dummy column to avoid com.github.murugeshkumark.dsa.array index issues
            dp[i][0] = 0;
        }

        for (int i = 0; i <= capacity; i++) {// Insert 0 in last row as it is dummy column to avoid com.github.murugeshkumark.dsa.array index issues
