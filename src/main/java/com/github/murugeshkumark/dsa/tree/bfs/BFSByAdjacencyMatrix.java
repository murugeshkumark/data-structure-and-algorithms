...
public class BFSByAdjacencyMatrix {
	...
	//BFS Algorithm
	void bfs() {
		for(GraphNode node: nodeList) { //if a com.github.murugeshkumark.dsa.node is unvisited then run com.github.murugeshkumark.dsa.tree.bfs on it
			if(!node.isVisited())
				bfsVisit(node);
		}
	}
	...
	void bfsVisit(GraphNode node) {
		...
		while(!queue.isEmpty()) {
			...
			for(GraphNode neighbor: neighbors) { 	//for each neighbor of present com.github.murugeshkumark.dsa.node
