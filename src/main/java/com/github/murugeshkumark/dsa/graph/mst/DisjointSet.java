...
public class DisjointSet {
    ...
    public static void driver(ArrayList<WeightedNode> nodeList) {
        makeSet(nodeList); //Create Disjoint Sets for each com.github.murugeshkumark.dsa.node in this list.
        for (int i = 0; i < nodeList.size() - 1; i++) {
            WeightedNode firstNode = nodeList.get(i);
            WeightedNode secondNode = nodeList.get(i + 1);
            System.out.println("Checking if com.github.murugeshkumark.dsa.node " + firstNode.getName() + " and " + secondNode.getName() + " belongs to different set, if yes, will Union them...");
    ...
    public static void makeSet(ArrayList<WeightedNode> nodeList) {
        //for each com.github.murugeshkumark.dsa.node in list, create a disjoint set
    ...
    public static DisjointSet union(WeightedNode node1, WeightedNode node2) {
        if (node1.getSet().equals(node2.getSet())) { //if two nodes are of same set then no union needed
            ...
            // if first set is bigger then update each com.github.murugeshkumark.dsa.node of second set to merge to set1
            if (set1.getNodes().size() > set2.getNodes().size()) {
                ArrayList<WeightedNode> nodeSet2 = set2.getNodes();
                for (WeightedNode node : nodeSet2) { //update each com.github.murugeshkumark.dsa.node of second set to merge to set1
                    node.setSet(set1);
                    set1.getNodes().add(node);
                }
                return set1;
            } else {
                // if second set is bigger/equal then update each com.github.murugeshkumark.dsa.node of first set to merge to set2
                ArrayList<WeightedNode> nodeSet1 = set1.getNodes();
                for (WeightedNode node : nodeSet1) {//update each com.github.murugeshkumark.dsa.node of first set to merge to set2
                    node.setSet(set2);
